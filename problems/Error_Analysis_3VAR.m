...
% close all
p.simulation.targetstate=statevar;
Save=1;
% randcount=29;
% deviation=.75;
% p.wt_estimate=0;
...
% p.cons = {'m', 'c', 'k';
%            2,  0.75,  3};
p.cons = {'a', 'b','c', 'dd','ee','ff','gg','hh','qo';
    -3, -2,-3,-3,1,-3,3,3,-1};
...
target = {'a*u(1)*u(3) +b*u(2)*u(3)+c*u(3)^2';
          'dd*u(1)*u(2)+ee*u(1)*u(3)+ff*u(2)*u(3)';
          'gg*u(1)*u(2) + hh*u(1)*u(3)+qo*u(2)*u(3)'};
...
% get target output
p = RunModel3(target,p);

p.simulation.ndata=3500;
...
if ~continuept2
    ...
    % nominal model
    if p.simulation.targetstate==1
        nom_mod.eqn_sym = a*x1+b*x2+c*x3;
    elseif  p.simulation.targetstate==2
        nom_mod.eqn_sym = dd*x1+ee*x3+ff*x2;
    elseif p.simulation.targetstate==3
          nom_mod.eqn_sym = gg*x1+hh*x3+qo*x2;      
    else
            disp('FAIL');
    end
...
if Save==1
    ...
    if p.wt_estimate==0 
        savefile = ['Data/3VAR_S' num2str(p.simulation.targetstate) '_NLS_randcount' num2str(randcount) '_mu' ...
            num2str(p.mod_adapt.mustart(1)*100) '_dev' num2str(deviation*100) '_its' num2str(its) '-' num2str(its2)]
    else
        savefile = ['Data/3VAR_S' num2str(p.simulation.targetstate) '_PARSIM_randcount' num2str(randcount) '_mu' ...
            num2str(p.mod_adapt.mustart(1)*100) '_dev' num2str(deviation*100) '_its' num2str(its) '-' num2str(its2)]
    end    
    save(savefile); 
    best_mod.eqn_sym
