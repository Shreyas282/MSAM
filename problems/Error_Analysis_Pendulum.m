...
%% Model Setup
syms w th
p.intvars = [w,th]; % internal variables of the system
p.absintvars = [abs(w),abs(th)];
...
% p.mod_adapt.exp = exp;
% target = {['']};
% star_mod.eqn_sym = c_a*a+c_w*w-c_t*theta;
...
% simulation options
    ...
    load('input_real_pend_a_1.mat');
    
    [target,kob] = shuffle(target);
    omega = omega(kob);
    theta = theta(kob);
    
    
    p.Y = target(1:round(end/2));
    p.w = omega(1:round(end/2));
    p.th = theta(1:round(end/2));
    
    sorter = [1:length(target)]';
    unsort(kob,1) = sorter;
    
    
p.simulation.ndata = length(p.w);
...
if ~continuept2
    ...
    % nominal model
    nom_mod.eqn_sym = -c_w/c_a*w-c_t/c_a*th;
    ...
    nom_mod.eqn_str = char(nom_mod.eqn_sym);
    ...
    p.mod_adapt.beta_start = .05*ones(num_terms,1);
...
%         375*(1+deviation*(random(1,randcount))), 9800*(1+deviation*(random(2,randcount))), 130000*(1+deviation*(random(3,randcount)))};
    ...
    try
        tmp = EvalModel({nom_mod.eqn_str},p);
       yhat = tmp.target;
    catch
...
%     nom_strdist = strdist(char(star_mod.eqn_sym),char(nom_mod.eqn_sym));
    ...
    plot(p.Y,'k'); hold on;
    plot(yhat,'xk');
    l=legend('$y^*(M^*,\Theta^*,t)$','Initial $\hat{y}(\hat{M},\Theta^*,t)$');
...
savefile = ['Data/Pendulum_NLS_mu' ...
            num2str(p.mod_adapt.mustart(1)*100) '_its' num2str(its) '-' num2str(its2)]

        % validation
unsort = unsort(round(end/2):end);
Yv = target(unsort);
p.Y = target(unsort);
p.w = omega(unsort);
p.th = theta(unsort);

best_v = EvalModel({char(best_mod.eqn_sym)},p);
nom_v = EvalModel({char(nom_mod.eqn_sym)},p);
% end
%% Plot Best
figure;
plot(p.Y,'b'); hold on;
plot(nom_v.target,'g');
plot(best_v.target,'r');
