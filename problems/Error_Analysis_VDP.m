...
%% Model Setup
syms v x m c k
p.intvars = [v, x]; % internal variables of the system
p.absintvars = [abs(v),abs(x)];
...
if ~continuept2
    ...
    % nominal model
    nom_mod.eqn_sym = -mu1*v+mu2*v-mu3*x;
...
%     deviation = devcnt;
    ...
    %randcount = 2;
    p.cons = {'mu1', 'mu2','mu3';
        1.5*(1+deviation*(random(1,randcount))), 1.5*(1+deviation*(random(2,randcount))), 1*(1+deviation*(random(3,randcount)))};
    
    try
        tmp = RunVDP2({nom_mod.eqn_str},p);
        if p.simulation.partitioning
            yhat = tmp.target;
        else
            yhat = tmp(:,4);
