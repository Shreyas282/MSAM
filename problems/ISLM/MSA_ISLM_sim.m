...
[YO,goodness] = fit([1:length(p.Y)]',p.Y,'cubicinterp');
[ydot,yddot] = differentiate(YO,1:length(p.Y));
...
p.simulation.u_in(:,1) = 1:p.simulation.samp_time:p.simulation.time_span; 
p.simulation.u_in(:,2) = islm.A(begin:finish,10);
...
if p.mod_adapt.algebra
    ...
    tmp = RunModel({p.nom_mod.eqn_str},p);
    p.yhat = tmp.target;
end
%     catch %#ok<*CTCH>
