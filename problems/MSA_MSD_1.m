...
% simulation options
p.simulation.sim_model='forced_msd_test';
...
if p.mod_adapt.algebra
     t = tmp.t;
    [FO,goodness] = fit(t,tmp.x1,'cubicinterp');
    ...
    p.u_in = tmp.u_in;
    p.x1 = tmp.x1;
%     tmp=load('nonlinmsd_derivatives_estimate.mat');
%     p.dx1 = tmp.dx1_in;
%     p.Y = tmp.ddx1_in;
...
if ~continuept2
    ...
    %% nominal model
    nom_mod.eqn_sym = (1/m.*(-c*dx1*abs(dx1)^(gamma(1)-1)-k*x1*abs(x1)^(gamma(2)-1)+u_in));
...
%    n = length(p.intvars)^num_terms; % number of tweaks to try (hill climbs)
    ...
    p.mod_adapt.mustart = repmat(1.0,num_terms,1); %starting confidence
  ...
  %% MC OPT
    ...
    p.mod_adapt.mc_opt2=0; % pertsize with dydtheta included
...
%     xlabel('t','fontsize',14);
    ...
    if p.mod_adapt.output_dydtheta
        dydtheta = zeros(128,3,3,num_terms,its);
        dydtheta_norm=zeros(128,3,3,num_terms,its);
    end
    mu=[];
    chosen=struct();
    pertnum=[];
        ...
        if pass(a)
            ...
            for x = 2:its
                ...
                try
                    ...
                    if p.mod_adapt.output_dydtheta
                        dydtheta(:,:,:,x-1,a) = outputs(x,a).dydtheta;
                        dydtheta_norm(:,:,:,x-1,a) = outputs(x,a).dydtheta_norm;
                    end
...
%                     if size(outputs(x,a).dydtheta,1)~= 0
%                         keyboard
%                     end
keyboard
                    pass2(x,a)=0;
...
if p.mod_adapt.output_dydtheta
        dydtheta2 = zeros(p.simulation.ndata,num_terms+1,size(p.cons,2),its);
        dydtheta_norm2=zeros(p.simulation.ndata,num_terms+1,size(p.cons,2),its);
    end
pause(.01);
for x = itsstart:its2
    pass3=1;
   % p.mod_adapt.algebra = 0;
    ...
    try
        ...
        [p_t2(:,x),phi2(:,:,x),error2(:,x),sum_abs_error2(x),max_corr2(x),outputs2(x),p,pass3] = Sensitivity_compute(best_models,beta2(:,x-1),gamma2(:,x-1),rho2(:,x-1),mu2(:,x-1),p_t2(:,x-1),p);
        if p.mod_adapt.output_dydtheta
            dydtheta2(:,:,:,x-1) = outputs2(x).dydtheta;
            dydtheta_norm2(:,:,:,x-1) = outputs2(x).dydtheta_norm;
        end
        if p.mod_adapt.MC_NLS_plot == 1
    ...
    if pass3
        % set outputs
        y02(:,:,x) = outputs2(x).y0;
        y_all2(:,:,:,x)=outputs2(x).y_all;
        y_ave2(:,:,x)=outputs2(x).y_ave;
        dMC2(:,x) = outputs2(x).dMC;
...
if Save==1
    ...
    if p.wt_estimate==0 
        savefile = ['Data_snapshot/MSD_NLS_randcount' num2str(randcount) '_mu' ...
            num2str(p.mod_adapt.mustart(1)*100) '_dev' num2str(deviation*100) '_its' num2str(its) '-' num2str(its2) '_' p.mod_adapt.mc_opt plus]
    else
        savefile = ['Data_snapshot/MSD_PARSIM_randcount' num2str(randcount) '_mu' ...
            num2str(p.mod_adapt.mustart(1)*100) '_dev' num2str(deviation*100) '_its' num2str(its) '-' num2str(its2) '_' p.mod_adapt.mc_opt plus]
    end    
    save(savefile); 
    best_mod.eqn_sym
...
clearvars -except rndcntr devcnt dvct opt initfinalplot p
end
% keyboard
end
end
...
subplot(1,3,[2 3]);
plot(p.Y,'k'); hold on;
l=legend('Final $\hat{y}(\hat{M},\hat{\Theta},t)$',...
    'Final $\hat{y}(\hat{M},\hat{\Theta},t)$',...
    'Final $\hat{y}(\hat{M},\hat{\Theta},t)$',...
    '$y^*(M^*,\Theta^*,t)$');
set(l,'interpreter','latex','fontsize',14);
xlabel('t','fontsize',14);
