...
out=[];
disp('\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\');
disp(' Running MSAMp version 1.0.1-wgl');
disp('///////////////////////////////');
...
if ~p.continuept2    
    ...
    num_models = length(p.intvars)^(p.num_terms/2); % number of models to try (hill climbs)
%     num_models=8;
...
if p.save==1
 ...
 %   savefile = [p.savepath '_its' num2str(its) '-' num2str(its2) '_Cm' num2str(p.simulation.Cm)];
