...
if ~p.continuept2    
    ...
    phi = zeros(p.simulation.ndata,p.num_terms,p.its,num_models);
    error = zeros(p.simulation.ndata,its);
    gamma=zeros(num_terms,its,num_models); %reset can_mod = nom_mod
    rho = zeros(num_terms,its,num_models);
    beta=[];
    p_t = zeros(num_terms,its,num_models);
    p_w = zeros(num_terms,its,num_models);
    delM = zeros(num_terms,its,num_models);
    mu=[];
    chosen=struct();
    pertnum=zeros(num_terms,num_models);
    models = zeros(num_terms+1,num_models);
    %% run hill climb
...
%    n=5;
       for a = 1:num_models
        ...
        while pass(a) ~= 1
                [chosen,can_mod,pertnum(:,a)] = ChoosePertType_sym(can_mod,p,pert_index(a,:));
