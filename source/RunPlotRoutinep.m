function perttitle = RunPlotRoutinep(h,a,x,its,y_ave,p,colors,...
...
%             set(gcf,'position',[200,200,1200,600]);
    ...
    plot(p.Y,'k'); hold on;
    try
    plot(y_ave(:,1),'-.xk','Linewidth',2); 
    catch
        keyboard
    end
    for count=1:length(y_ave(1,2:end,1))
       plot(y_ave(:,count+1),'linestyle','--','color',colors(count));
    ...
    xlim([0 its+1]);
    ylim([0 max([sum_abs_error(x);max_corr(x)])]);
